...
function render(items){
  ...
  items.forEach(item=>{
    ...
    card.append(head,stay,period,links);results.append(card);
  });
}
...
form.addEventListener("submit",async e=>{
  ...
  const params=new URLSearchParams({city:String(fd.get("city")||"").trim(),from:String(fd.get("from")||""),to:String(fd.get("to")||""),bedrooms:String(fd.get("bedrooms")||"1"),sleeps:String(fd.get("sleeps")||"1")});
