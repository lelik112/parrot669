...
function render(items){
  ...
  items.forEach(item=>{
    ...
    card.append(head,owner,stay,period,price,priceNote,links);results.append(card);
  });
}
...
form.addEventListener("submit",async e=>{
  ...
  const params=new URLSearchParams({city:"Barcelona",from:String(fd.get("from")||""),to:String(fd.get("to")||""),bedrooms:String(fd.get("bedrooms")||"1"),sleeps:String(fd.get("sleeps")||"1"),pricedOnly:fd.get("pricedOnly")?"true":"false"});
