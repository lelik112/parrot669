...
[PM-044](tasks/PM-044-qa-mailboxes.md) / P1 planned: четыре отдельных ящика для `qa`, `lelik`, `qa2`, `qa3`. Никита может подготовить все четыре после выбора сервиса; Борис принимает свою пару после привязки, Денис проверяет и привязывает адреса существующих тестовых аккаунтов после PM-040; claims отсутствуют. Алексей нужен лишь для выбора сервиса и прав/проверки человека, если сервис попросит. [PM-017](tasks/PM-017-qa-test-mailbox-access.md) остаётся blocked до подтверждённого доступа, затем Борис делает [PM-006](tasks/PM-006-email-acceptance.md). Личные ящики владельца не используются.

## PM-043 / 2026-09-26

[PM-043](tasks/PM-043-agent-wake-up-feasibility.md) закрыта Марком как исследование: одноразовый scheduled run в существующем чате Игоря и GitHub CAS прошли; пятиминутные запуски и триггер на Markdown не доказаны. Постоянное расписание не включено; назначения передаются вручную. Следующий шаг автоматизации — [PM-045](tasks/PM-045-five-minute-agent-wake-up-pilot.md), P2 blocked: callable Automations ограничен часом, отдельный Cloud Browser ChatGPT требует безопасного входа владельца; пилот ещё не запускался. [PM-037](tasks/PM-037-search-date-range-bound.md) выпущена Игорем после PM-043: backend/ frontend CI зелёный, Railway SUCCESS и live UI smoke; независимая QA Бориса открыта.
...
7. [PM-034](tasks/PM-034-backend-frontend-refactoring-audit.md) закрыта как аудит. Никита — кандидат на QA узкого сценария [PM-038](tasks/PM-038-host-unsaved-drafts-rerender.md) без зависимости от PM-036; после отдельного claim Денис может взять исправление. Игорь сейчас ведёт [PM-043](tasks/PM-043-agent-wake-up-feasibility.md) (Sol / High); [PM-037](tasks/PM-037-search-date-range-bound.md) возобновит после пилота: свежие PR/CI → backend/frontend release → QA Бориса.
