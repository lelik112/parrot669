...
- **Новая роль [Regression QA Engineer](decisions/D013-qa-acceptance-regression-split.md):** утверждена область smoke/регресса; агент ещё не назван и не подключён, QA claims и задачи не переносились. Борис остаётся QA Lead / Acceptance QA. [Предложенное разделение очереди](tasks/PM-030-qa-drain-next-stage.md#qa-split--новая-роль-2026-09-25-после-появления-исполнителя) заработает после появления человека и отдельных task claims.
...
2. После появления и доступа Regression QA взять ограниченные UI/локализационные ретесты из [QA split PM-030](tasks/PM-030-qa-drain-next-stage.md#qa-split--новая-роль-2026-09-25-после-появления-исполнителя) с отдельными claims и evidence; Борис продолжает критический acceptance. Пока исполнителя нет, распределение задач не меняется.
3. Борис идёт по [приоритетной QA-очереди PM-030](tasks/PM-030-qa-drain-next-stage.md#одна-очередь-бориса-после-pm-029pm-016): сначала PM-024/003, затем PM-023, PM-002, PM-025/001; короткие P2 ретесты и внешние условия отмечены отдельно.
4. Алексей решает вопросы публичной видимости [PM-022](tasks/PM-022-public-host-profile.md); после первых результатов QA Марк рекомендует **одну** следующую dev-задачу. Текущий условный кандидат — PM-022, лишь после принятого контракта. PM-031 — редакционная подготовка; PM-032/033 — LATER.
5. После текущих P1 проверок Игорю и Денису отдельно claim-ить read-only [PM-034-BE](tasks/PM-034-BE-backend-audit.md) и [PM-034-FE](tasks/PM-034-FE-frontend-audit.md), проверить гипотезы и передать отчёты Марку; рефакторинг кодом не запускать.
6. Готовить [PM-008](tasks/PM-008-pilot.md) как отдельный план измерения реальных добавлений, полезной выдачи и контакта без выдуманных базовых чисел.
