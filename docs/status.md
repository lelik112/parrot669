...
Дата среза: **2026-09-25**. Автор: **Марк / PM**. Дополнение PM-024 от **2026-09-25**: **Игорь / Developer** (релиз и проверка live UI; независимая QA ещё открыта).
...
## План разработки — 2026-09-25

- **Денис — BUG-020 / P2:** назначена диагностика непрочитанного состояния. Claim и Model check ожидаются; реализация не должна конфликтовать с PM-027.
- **Игорь — PM-027 / P2:** in progress; frontend handoff/CI/live release/QA ещё впереди.
- **PM-028 Дениса:** frontend выпущен, in review; host/guest и mobile QA открыты. **PM-025:** web-исправление опубликовано и live проверено, mobile и независимая QA открыты.
- В BUG-020 сначала проверяются unread API, polling и UI. Общие Messages-файлы не менять до handoff PM-027 или согласования области с Игорем.
...
D009 кабинета подтверждён владельцем. Последовательность принята: PM-020 → PM-021 → PM-022. PM-020 уже реализован Денисом и находится в review; PM-021 — следующий этап, пилот его не блокирует; PM-022 следует после PM-021 с отдельным privacy/publication контрактом.

## Decisions

- [D011](decisions/D011-contact-always-available.md): каждый объект, оставленный в поиске, имеет PARROT contact path; host-wide opt-in не блокирует первое обращение. PM-024 Игоря выпущена в оба main, Railway SUCCESS и живой UI проверен; независимая двухаккаунтная QA Бориса открыта.
...
- [D009](decisions/D009-host-cabinet-ia.md): минимальная IA кабинета accepted; последовательность PM-020 → PM-021 → PM-022. Private Profile не ждёт пилота.
...
- PM-024 выпущена: legacy host opt-in не скрывает контакт и не блокирует первое обращение; CI/Railway/live UI подтверждены. Проверка первого сообщения и ответа двумя аккаунтами, отказа при блокировке пары и rate limits независимо Борисом ещё открыта.
...
1. Борису проверить PM-024 двумя аккаунтами: контакт из любого результата → первое сообщение → ответ, legacy opt-out, auth, блокировка пары и rate limits; реализация Игоря выпущена.
...
3. Борис продолжает PM-003 и подтверждает реальный путь поиска и контакта; отдельно берёт следующий QA этап PM-004/005, если доступ и claim подтверждены.
4. PM-002 разблокирована D010: назначить реализацию внешней ссылки + честного verification status + messaging nudge; Airbnb остаётся необязательным для поиска.
5. Борис независимо проверяет [PM-010](tasks/PM-010-auth-routes-extraction.md) по auth UI/сессиям; Игорь выпустил [PM-011](tasks/PM-011-calendar-sync-extraction.md); Борис взял независимый календарный QA в очередь. PM-012 остаётся LATER.
6. Довести QA PM-020; затем брать PM-021 как следующий этап кабинета. PM-022 — после PM-021 с отдельным privacy/publication contract. Пилот не блокирует PM-021.
7. Выполнить PM-023 после PM-024: дать гостю составить обращение до auth и восстановить черновик после входа/регистрации.
8. PM-025 в работе у Дениса; mobile не считать дефектным до отдельной проверки. PM-026: frontend-изменение готово и ждёт независимый QA; глобальная Messages-ссылка остаётся.
9. PM-027 в работе у Игоря; PM-028 выпущена Денисом и ждёт QA. BUG-020: Денис диагностирует unread state; кодовые изменения — после handoff PM-027.
