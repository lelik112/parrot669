...
- [PM-016](tasks/PM-016-qa-second-session-access.md) / P1: Борис независимо подтвердил два управляемых входа, контакт, ответ, reload и изоляцию logout `qa`; задача done. Полная PM-003 остаётся открытой.
...
- **[PM-003](tasks/PM-003-contact-acceptance.md), `in progress`:** базовый поиск → сообщение → ответ PASS; продолжаются блокировка пары и повторное обращение. Это работа Бориса, не инфраструктурный blocker.
...
- Базовый путь «поиск → сообщение → ответ» подтверждён независимым двухаккаунтным smoke; отрицательные сценарии и ценность для реальных пользователей ещё не приняты. Очередь QA растёт быстрее независимой приёмки.
...
1. Борис завершает PM-029 security/reverse logout и PM-003 pair block/repeat contact; базовый A→B→A уже PASS. Игорь чинит только подтверждённые дефекты в своём scope.
