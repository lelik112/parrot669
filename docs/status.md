...
- [D011](decisions/D011-contact-always-available.md): каждый объект, оставленный в поиске, имеет PARROT contact path; host-wide opt-in не блокирует первое обращение. PM-024 поручена Игорю; claim и Model check ожидаются.
...
- В production legacy host opt-in пока может скрывать контакт. D011 уже принят, но PM-024 ещё не выпущена; до её релиза это остаётся известным разрывом между продуктовым решением и текущим поведением. Сохраняются авторизация, rate limits и блокировка участника.
...
1. Игорю подтвердить claim/Model check PM-024 и реализовать D011: контакт из каждого объекта в поиске, независимо от legacy host opt-in; auth, rate limits и блокировки остаются.
