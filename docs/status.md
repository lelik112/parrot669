...
- **[PM-036](tasks/PM-036-regression-qa-independent-two-accounts.md) / P1, in review:** Никита независимо прошёл A→B→A, refresh, logout/relogin обоих аккаунтов `qa2`/`qa3` на выделенных Worker origin. Функциональный доступ доступен для назначенного ручного регресса. Для закрытия Борис / QA Lead (Sol / Medium) подтверждает сохранность собственной пары, Игорь / Developer (Sol / High) дополняет live negative/security evidence новых адресов; Марк / PM принимает оба отчёта.
...
3. [PM-036](tasks/PM-036-regression-qa-independent-two-accounts.md): Никитин функциональный A↔B PASS. Игорь / Sol / High дополняет read-only negative/security проверки новых origin, Борис / Sol / Medium подтверждает свою пару; Марк закрывает задачу после обоих отчётов.
