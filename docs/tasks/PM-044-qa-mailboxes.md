...
**Status:** blocked / Gate 3 QA — письма восстановления для `qa`, `qa2`, `qa3` не доставлены; подключённый Resend не показывает вызова отправки для этих recovery-запросов, причина до token-state/lookup/rate-limit проверки не установлена; самостоятельный login QA-пар открыт; Gate 1 PASS; Gate 2 PASS: четыре существующих QA-аккаунта атомарно перепривязаны к четырём уникальным AgentMail plus-адресам, immutable ID/allowlist сохранены, post-change verify и healthy cleanup deploy пройдены. Борис принимает PM-017/PM-006, Никита делает регресс своей пары. **Priority:** P1 / NEXT, разблокирует [PM-017](PM-017-qa-test-mailbox-access.md) и [PM-006](PM-006-email-acceptance.md).
...
## Resend event check — 2026-09-26T13:24:37.697Z

**Agent:** Денис. **Role:** Developer. **Wake ID:** `PM-044-GATE3-DELIVERY-TRIAGE-20260926-1254` (continuation of signed claim). **UTC:** 2026-09-26T13:24:37.697Z. **Source:** [PR #62 comment #5846446635](https://github.com/lelik112/parrot669/pull/62#issuecomment-5846446635). **Related task:** PM-044.

**Read-only evidence:** после подключения владельцем Resend я проверил account API logs и sent-email metadata. В подключённом аккаунте есть verified/sending-enabled domain `parrot669.com`, прежние успешные `POST /emails` от Java 21 и доставленные PARROT-письма до 2026-09-25 18:57 UTC. Последний такой POST — 2026-09-25 18:57:36 UTC; в списках нет `POST /emails` и новых отправленных писем за 2026-09-26, включая окна production recovery POST 12:51:47, 12:51:57 и 12:53:23 UTC. Поэтому эти три UI 202 не подтверждают обращение к Resend. Отсутствие события в подключённом аккаунте с историей PARROT strongly suggests, что send не был вызван (например, lookup/rate limit/queue worker branch), но без token-state и проверки используемого production key нельзя полностью исключить другой Resend account/key. Адреса, токены и содержимое писем не раскрыты.

**BLOCKED / exact next action:** нужна read-only проверка состояния `password_reset_tokens` и веток lookup/rate limit для трёх попыток, без значений token/email и без изменения accounts; сопоставить production key с подключённым Resend account без раскрытия ключа. Если нужен код/ops для наблюдаемости либо исправления, до изменения отдельный scoped claim/план. При установлении причины — исправление в согласованном scope и новый независимый Gate 3 QA, затем безопасный reset/login тестеров. Алексей сейчас не должен пересылать credentials или повторять запросы; возобновление после read-only token evidence.


## PM review — PM-044-RESEND-NO-POST-20260926-1324

**Agent:** Марк. **Role:** Product Manager. **Wake ID:** `PM-044-RESEND-NO-POST-20260926-1324`. **UTC:** 2026-09-26T13:26:09.422Z. **Source:** [PR #60 comment #5846630998](https://github.com/lelik112/parrot669/pull/60#issuecomment-5846630998). **Related task:** PM-044 / PM-017 / PM-006.

**Result:** signed read-only Resend check Дениса подтверждает отсутствие `POST /emails` и sent-event в подключённом аккаунте на три production recovery запроса; прежние PARROT отправки и verified domain там есть. Это сужает диагноз к веткам до provider send, но другой production account/key ещё не исключён. Следующий шаг в scope Дениса — read-only token-state/lookup/rate-limit и сверка account без раскрытия ключа; до доказательства причины никакого фикса и повторного QA. Подключение Resend уже выполнено, от Алексея сейчас действия не требуется, поэтому owner-alert письмом не отправляется. PM-044 остаётся blocked; Денис независимо взял PM-049.
