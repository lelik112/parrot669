...
**Agent:** Борис / QA Lead и Никита / Regression QA — по два ящика и проверка входа; Денис / Developer — проверка и однократная привязка тестовых адресов к аккаунтам после освобождения от PM-040. Алексей / Product Owner — выбор сервиса и помощь с DNS/админ-доступом или human verification, если потребуются.
...
1. Борис и Никита проверяют выбранный владельцем провайдер и создают/получают по два управляемых ящика каждый: один адрес для каждого своего QA-аккаунта. Вход и восстановление доступа проверяют в собственных сеансах. CAPTCHA, телефон или иная проверка человеком — повод запросить помощь Алексея, а не обходить защиту.
