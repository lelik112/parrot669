...
- 2026-09-26T14:08:30Z — **Agent:** Денис. **Role:** Developer. **Wake ID:** `PM-049-DENIS-BFCACHE-FIX-20260926-1357` (result of signed claim). **Source:** [PR #62 comment #5846845113](https://github.com/lelik112/parrot669/pull/62#issuecomment-5846845113). **Related task:** PM-049. **Change:** [frontend PR #69](https://github.com/lelik112/parrot669/pull/69) merged as `ad50ab79fd54e97c042b9e37c2325c788643bc68`. On persisted `pageshow`, Host now synchronously clears username, private email, profile controls and property DOM before a fresh `/auth/me` + dashboard rebuild; shared Search/Messages logic synchronously drops cached account/private conversation state before its fresh session check. Ordinary first load and non-BFCache navigation are unchanged; backend, auth API, data, PM-044 and PM-050 were not touched.

  **Verification:** local `npm test` 143/143 PASS, including new focused Host/Search/Messages checks that assert sensitive DOM is gone before session revalidation completes. Local Playwright could not launch because browser binaries are absent in the worktree, not because a case failed. [Frontend CI run #36247319404](https://github.com/lelik112/parrot669/actions/runs/36247319404) PASS: unit job and browser-smoke 21/21 across Chromium desktop, WebKit desktop and mobile emulation. Cloudflare reported successful branch/commit-preview deployment for PR #69; canonical Host opened after merge in anonymous state. This does not substitute for the exact signed-in logout → browser Back acceptance path.

  **Status / next:** in review. Борис / QA Lead to independently repeat the original signed-in Host#host-profile → Search logout → browser Back sequence on the deployed origin and confirm that no username, private email, property or edit controls appear before/after revalidation; also spot-check Search and Messages history restores. Real iPhone/Safari remains PM-041.
