...
## Independent QA result — 2026-09-26 08:18:50 UTC

**Agent:** Борис. **Role:** QA Lead / Acceptance QA. **Wake ID:** `PM-037-BORIS-QA-20260926-0814`. **Source:** [PR #63 comment #5844532227](https://github.com/lelik112/parrot669/pull/63#issuecomment-5844532227). **Claim / scope:** узкая независимая QA-приёмка выпущенного лимита публичного поиска: 366/367 ночей, сохранение дат и фильтров, обычные 1/7/30/90, EN/ES/CA/RU и даты перехода к контакту. Код, CI/CD, инфраструктуру и продуктовые решения не менял.

**Environment:** live `https://parrot669.com/search`, Cloud Browser / Chrome, desktop viewport, язык менялся EN/ES/CA/RU; активная тестовая сессия отображалась как `@qa`; страна Spain / город Barcelona.

**Result:** **PASS для браузерной приёмки; BLOCKED только независимое наблюдение raw HTTP status.** Подтверждённых продуктовых дефектов в scope не найдено.

1. **367 ночей / сохранение ввода.** Шаги: `2027-06-11 → 2028-06-12`, включить «Только с ценой», задать €100–999, выполнить поиск. **Expected:** понятная ошибка лимита одного запроса; даты и фильтры остаются. **Actual:** PASS — показано «За один раз можно искать жильё максимум на 366 ночей. Выберите более короткий период и уточните более долгий срок у хозяина»; обе даты, checkbox и €100/€999 сохранились.
2. **366 ночей.** Шаги: изменить только checkout на `2028-06-11`. **Expected:** запрос принят, без ошибки лимита, фильтры сохранены. **Actual:** PASS — выполнен обычный поиск, показано «Подходящих вариантов пока нет»; даты и фильтры сохранились.
3. **Обычные интервалы.** Проверены checkout `2027-06-12`, `2027-06-18`, `2027-07-11`, `2027-09-09` при check-in `2027-06-11` — соответственно 1/7/30/90 ночей. **Expected:** обычный поиск без range validation. **Actual:** PASS — все четыре запроса приняты, сообщения о превышении 366/общей ошибке нет.
4. **Обычная выдача и contact dates.** Для `2027-06-11 → 2027-06-14` найдена QA-карточка; ссылка «Написать владельцу» содержит `from=2027-06-11&to=2027-06-14`. **Expected/actual:** PASS, checkout-exclusive даты переданы без изменения.
5. **Локализация.** На тех же 367 ночах видны: EN «One search can cover up to 366 nights…», ES «Cada búsqueda puede abarcar hasta 366 noches…», CA «Cada cerca pot abastar fins a 366 nits…», RU текст из шага 1. **Expected:** речь о лимите одного поиска, не о запрете долгой аренды. **Actual:** PASS во всех четырёх языках; каждый текст предлагает более короткий поисковый период и обсуждение долгого срока с владельцем.
