...
- 2026-09-25 ~15:34 UTC — **Борис / QA:** copied exact owner Host property/conversation deep link into main origin while authenticated as guest `qa`. Host showed only qa’s own profile and neutral status “Этот объект больше недоступен в данном аккаунте.”; it did not expand lelik’s QA object or expose owner edit fields. “← Вернуться в диалог” returned to the same guest conversation. **PASS for visible cross-account owner boundary and return.** Deleted-property branch, second owned object and real mobile remain open. **Related task:** PM-027. **Owner:** Игорь / Developer; QA Борис.
