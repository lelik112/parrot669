...
### 2026-09-24 14:14 UTC — Борис / QA: production lifecycle часть

- **Agent:** Борис. **Role:** QA. **Scope:** аккаунт `lelik`, тестовый Airbnb календарь в production desktop Chrome; ручной sync и disable/enable через UI. **Status:** in review, полная приёмка открыта.
- **Change:** ручное «Обновить сейчас» сдвинуло последнюю синхронизацию с 16:30:23 на 17:11:33, брони 0 / игнорируемые блокировки 1. «Выключить» показало статус «Airbnb · Выключен» и сделало sync недоступным; «Включить» вернуло «Airbnb · Подключён», sync до 17:11:55, счётчики 0/1. Верификация осталась в состоянии failed после первой PM-013 попытки — disable/enable не выдали ложный успех. **PASS для проверенных UI-действий.** Календарь оставлен включённым.
- **Related task:** PM-011, PM-001, PM-018.
- **Ограничение:** удаление/повторное подключение не выполнялись Борисом в этом прогоне: для безопасного восстановления нужна секретная экспортная iCal-ссылка. Предыдущее переподключение выполнял Алексей. Guest search при toggle, точные HTTP-контракты, SSRF и классификация feed не подтверждаются этим UI-прогоном. Доказательство `parrot-pm011-enabled-20260924.jpg` предоставлено владельцу; пока не в GitHub.
- **Next:** с подготовленной ссылкой и тестовым источником проверить delete/connect и guest search; не объявлять всю PM-011 принятой по sync/toggle.
