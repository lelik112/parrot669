...
Репозиторий не будит агентов и не обеспечивает автоматическое прочтение. Агент проверяет задачу при старте/возобновлении, перед пересекающимся изменением и перед передачей результата. Ответ появляется отдельной подписанной записью. PM не утверждает, что задание получено, пока нет подтверждения. С 2026-09-26 в существующем чате Марка включён почасовой PM check-in: он читает новые записи и реагирует на конфликты/блокеры без ручного пинга Алексея. Это расписание **не будит** чаты разработчиков и QA; отдельное исследование событийного пробуждения — [PM-046](PM-046-github-event-existing-chat-wakeup.md).
