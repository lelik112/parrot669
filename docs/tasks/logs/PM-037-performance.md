...
Марк принял замер для **малого пилота**; при 500 объектах/город и четырёх запросах p95 2,216 с остаётся риском масштабирования. Игорь проверяет frontend PR #52 (`mergeable=false`) и current-head CI, затем merge backend → frontend, фактический Railway/Cloudflare deploy и передача независимой QA. Изолированная проверка не является production SLO и не подтверждает, что релиз уже выполнен.
