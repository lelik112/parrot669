...
Марк подтверждает допустимость 366 на выбранном пилотном объёме либо уточняет D014/узкую оптимизацию для большего объёма. После этого merge обоих PR, фактический Railway/Cloudflare deploy и независимая QA. До решения PM изменения в production не объявляются готовыми.
