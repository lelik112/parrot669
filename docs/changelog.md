...
## 2026-09-26 — D017 / PM-046: адресные PR inbox для всей команды

- **Agent:** Марк. **Role:** Product Manager. **Change:** после подтверждённого двустороннего comment handoff Марк ↔ Игорь и второго отдельного запуска Игоря владелец поручил подготовить inbox для всей команды. Открыты docs-only draft PR [Денис #62](https://github.com/lelik112/parrot669/pull/62), [Борис #63](https://github.com/lelik112/parrot669/pull/63), [Никита #64](https://github.com/lelik112/parrot669/pull/64), [Алекс #65](https://github.com/lelik112/parrot669/pull/65); правила и статусы записаны в [D017](decisions/D017-pr-inbox-handoffs.md), [workflow](tasks/workflow.md#адресные-pr-inbox--d017--pm-046), [team](team.md#адресные-pr-inbox) и [PM-046](tasks/PM-046-github-event-existing-chat-wakeup.md). Эти четыре адреса пока `prepared`: личные triggers и доставка ждут однократной настройки в соответствующих существующих чатах. Код и инфраструктура не менялись. **Related task:** PM-046.

## 2026-09-26 — PM-046: подготовлен адресный PR inbox Игоря

- **Agent:** Марк. **Role:** Product Manager. **Change:** после успешного комментария → существующий чат Марка в PR #60 подготовил docs-only draft [PR #61](https://github.com/lelik112/parrot669/pull/61) и инструкцию однократной настройки `enable_comments` в прежнем чате Игоря. Карточка PM-046 и статус обновлены. Игорь включил `enable_comments` в прежнем чате для PR #61; адресный комментарий с уникальным Wake ID разбудил его, одна CAS-запись в PM-046 появилась через 96 с от публикации. Ограниченный пилот канала к Игорю разрешён. Ответный [комментарий Игоря в PR #60](https://github.com/lelik112/parrot669/pull/60#issuecomment-5843756979) запустил прежний чат Марка и привёл к отдельной подписанной записи PM-046. Оба направления проверены по одному разу; дубли и регулярная стоимость ещё не проверены. Старый PR #59 закрыт, его пилот выключен. **Related task:** [PM-046](tasks/PM-046-github-event-existing-chat-wakeup.md).
...
## 2026-09-26 — событийное пробуждение остаётся открытой целью

- **Agent:** Марк. **Role:** Product Manager. **Change:** отделил доказанный одноразовый запуск PM-043 и закрытый пятиминутный пилот PM-045 от непротестированного GitHub-варианта. Создал [PM-046](tasks/PM-046-github-event-existing-chat-wakeup.md) для проверки push/PR event именно для существующего чата; Игорь предложен исполнителем, claim пока нет. Нового агента и автоматизации не создавали. **Related tasks:** PM-043, PM-045, PM-046.

## 2026-09-26 — PM-045 закрыта после проверки частоты

- **Agent:** Марк. **Role:** Product Manager. **Change:** Алексей передал вывод Игоря по PM-043: повтор каждые пять минут настроить не удалось. Закрыл [PM-045](tasks/PM-045-five-minute-agent-wake-up-pilot.md) как недоступный в проверенной конфигурации пилот, без расписания, запусков и измерения расхода. Одноразовый запуск PM-043 остаётся доказанным; задачи передаются вручную. Возможности всех вариантов авторизованного UI отдельно не подтверждались. **Related tasks:** PM-043, PM-045.

## 2026-09-26 — PM-037 search range released

- **Agent:** Игорь. **Role:** Developer. **Change:** [backend PR #29](https://github.com/lelik112/parrot669-backend/pull/29) merged as `90a4d90`; [CI 36218244078](https://github.com/lelik112/parrot669-backend/actions/runs/36218244078) PASS and Railway production deployment `63815f47` SUCCESS. Frontend [PR #52](https://github.com/lelik112/parrot669/pull/52) updated against current main and merged as `17a6d9c`; [CI 36218605734](https://github.com/lelik112/parrot669/actions/runs/36218605734) PASS. Live search displays the 367-night explanation while retaining dates, accepts 366 nights, and still finds the ordinary 3-night listing. Борис независимо подтвердил 366/367, 1/7/30/90, четыре языка, сохранение дат/фильтров и contact dates; PM-037 закрыта Марком. Raw production HTTP 400 не считан из Cloud Browser из-за `ERR_BLOCKED_BY_CLIENT`, но server-before-SQL покрыт route tests/benchmark; production load SLO не заявлен. **Related task:** [PM-037](tasks/PM-037-search-date-range-bound.md) / D014.
