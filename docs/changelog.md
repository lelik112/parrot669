...
- **Agent:** Марк. **Role:** Product Manager. **Change:** Зафиксирована задача P1 на гостевой путь из поиска к первому сообщению: подготовить обращение до входа, потребовать auth перед отправкой, вернуть пользователя к тому же объекту/датам/черновику и сохранить приватность inbox. Кнопка «Написать владельцу» уже есть в поиске и показывается при включённых новых обращениях хозяина. Исполнитель не назначен; задача в NEXT после текущих PM-020/021 claims. **Related task:** [PM-023](tasks/PM-023-guest-message-onboarding.md).
