...
13. Production still contains a host-wide **Guest messages** opt-in, but accepted [D011](decisions/D011-contact-always-available.md) says it must not block a first message about a property kept in search. [PM-024](tasks/PM-024-contact-from-every-search-result.md) removes or repurposes this legacy gate. Email notification preferences remain separate. The Messages tab opens the same inbox used by guests, with an unread badge.
