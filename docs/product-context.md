...
## PM-048 — concise search copy and usable price-only control (production released; independent review pending)

**Agent:** Игорь. **Role:** Developer. **Related task:** [PM-048](tasks/PM-048-search-clarity-and-price-toggle.md) / PM-047. The standalone search keeps its date, capacity, accommodation and total-price semantics, but no longer renders the three redundant explanatory rows below the form. Their unused translation keys, wrapper styles and `aria-describedby` references were removed with them.

The lead now says only that the guest can find a place available for the selected dates in EN/ES/CA/RU; it does not imply booking, contractual verification or that every property has an external listing. The existing price-only label remains the clickable control, with a 22 px checkbox, 52 px desktop / 56 px mobile hit area and visible keyboard focus. Unit coverage checks all four translations, removed accessibility references and label toggling; the focused Playwright scenario checks keyboard operation, rendered dimensions and overflow in desktop Chromium/WebKit and mobile emulation. Local unit tests pass 138/138; browser execution was delegated to CI because this environment has no Playwright browser binaries. [PR #67](https://github.com/lelik112/parrot669/pull/67) merged as `61563d2`; PR CI [36244195339](https://github.com/lelik112/parrot669/actions/runs/36244195339) and main CI [36244332329](https://github.com/lelik112/parrot669/actions/runs/36244332329) passed, including all 21 browser cases across three projects. Live asset smoke confirmed the concise lead and enlarged-filter CSS on both `parrot669.com` and the Workers production origin. Independent web review remains pending; native iPhone remains PM-041.
...
The narrow frontend fix retries the saved position during a bounded three-second layout-settling window, always clamps to the currently reachable maximum and stops as soon as the target is reached. Wheel, touch, pointer or keyboard input cancels restoration immediately, so it cannot fight a user who starts navigating. Ordinary reload, profile/property deep links, auth, drafts and API data are unchanged. A controller regression simulates the observed 3614 px late document growth and reaches the saved 2678 px. [PR #66](https://github.com/lelik112/parrot669/pull/66) merged as `844bb63`; PR CI [36229648159](https://github.com/lelik112/parrot669/actions/runs/36229648159) and main CI [36229732675](https://github.com/lelik112/parrot669/actions/runs/36229732675) passed, including 137 unit checks and 18 browser scenarios. Both `parrot669.com` and the Workers origin serve the new helper. Boris independently confirmed exact deep and shallow restoration, expanded-card state and one-shot reload behavior on both production origins. BUG-022 is accepted and closed; touch/mobile remains separate PM-015/PM-041 scope.

## QA direction — D016 / PM-040 / PM-041 (2026-09-25)

**Agent:** Марк. **Role:** Product Manager. **Related tasks:** [PM-040](tasks/PM-040-playwright-web-e2e.md), [PM-041](tasks/PM-041-real-device-qa-queue.md), [D016](decisions/D016-playwright-first-real-device-queue.md). Алексей выбрал Playwright для повторяемых браузерных проверок основного пути. Денис выпустил два этапа PM-040 (PR #54 и #57 merged; 4 контролируемых fixture-сценария × 3 browser projects, последний PR CI 12/12); матрица критериев обновляется по фактическому покрытию, независимая QA и дальнейшее web покрытие открыты. Никита отдельно прошёл ручной двухаккаунтный smoke PM-036 (A→B→A, refresh, logout/relogin), Борис подтвердил сохранность собственной пары; PM-036 закрыта по решению владельца с явно непроверенным live отказом постороннему подтверждённому аккаунту на QA origin. Автоматический auth E2E требует отдельного изолированного окружения. Нативный iPhone Safari, экранная клавиатура, date picker и сохранённый адрес Safari остаются отдельными blocked критериями с [инструкцией для QA и очередью PM-041](tasks/PM-041-real-device-qa-queue.md). Playwright WebKit/mobile emulation не является независимой real-device приёмкой. Борис отвечает за критический acceptance, Никита — за назначенный регресс; рабочие результаты хранятся в исходных task cards. Исследование выбора [PM-039](tasks/PM-039-browser-test-environments.md) закрыто; покупка устройства/сервиса сейчас не запущена.
...
## PM-037 — search range protection (done after independent QA)
...
The search UI explains the request limit in EN/ES/CA/RU, preserves dates and filters, and never substitutes a shorter stay. Boundary, restoration, language switching and date propagation to contact are tested. Isolated PostgreSQL measurements and both CI checks passed: 366-night full HTTP p95 is 226 ms for 100 city properties / 1,044 ms for 500; four concurrent requests yield 572 / 2,216 ms. See the [report and plans](tasks/logs/PM-037-performance.md). Mark accepted these isolated measurements as the performance gate for a **small pilot** on 2026-09-25; 500-city-property p95 of 1,044 / 2,216 ms remains a scaling risk, not a production SLO. Backend [PR #29](https://github.com/lelik112/parrot669-backend/pull/29) merged as `90a4d90` and reached Railway production deployment `63815f47` SUCCESS on 2026-09-26; [main CI 36218244078](https://github.com/lelik112/parrot669-backend/actions/runs/36218244078) PASS. Frontend [PR #52](https://github.com/lelik112/parrot669/pull/52) was merged after updating it against main as `17a6d9c`; [main CI 36218605734](https://github.com/lelik112/parrot669/actions/runs/36218605734) PASS. Live `parrot669.com/search` showed the new Russian 367-night explanation without losing the selected date, accepted 366 nights with an empty result, and returned the existing 3-night listing. The Cloudflare dashboard was blocked by its browser security check, so exact Worker version ID was not read; visible live behavior confirms the new UI is served. Direct API navigation was blocked by the client; backend 400-before-repository is confirmed by route tests and benchmark, not a live raw-API request. Boris independently confirmed the live 366/367-night behavior, ordinary 1/7/30/90-night searches, EN/ES/CA/RU wording, date/filter retention and contact dates; PM-037 is done. Raw production HTTP 400 was blocked by Cloud Browser; route tests and the isolated benchmark cover the server rejection before SQL.
...
- [Regression QA two-account procedure](qa/pm036-testers.md): use after the two dedicated Workers and QA account IDs have been activated and live-checked. Both Workers and secrets are live; the owner-approved `qa2`/`qa3` immutable IDs are provisioned, and both accounts remain independently signed in after reload. Messaging A↔B, logout/relogin and independent QA remain open.
...
Before starting or resuming work, read the current status, short task index, relevant task and new addressed messages in today's team-chat log. Consult relevant product-context sections and accepted decisions for product choices, team/AI rules for role or model questions, and related changelog entries for historical claims. Full archives remain available by link. State **Agent, Role, Scope** and record a claim before implementation; do not start a duplicate task already claimed by another agent. Every commit message, PR description, task-status update and changelog entry identifies **Agent, Role, Change, Related task**, even when the technical GitHub account is shared. Ordinary repository updates are asynchronous records and do not start another agent. Under [D017](decisions/D017-pr-inbox-handoffs.md), an addressed comment in an open PR with a personal enabled GitHub trigger can start that agent's existing chat; [team.md](team.md#адресные-pr-inbox) distinguishes active inboxes from prepared PRs. The task card on main remains authoritative for assignment, scope, claim and evidence. The workflow is in [tasks/workflow.md](tasks/workflow.md); the canonical team roster and formats are in [team.md](team.md).

Mark / Марк is PM and edits product documentation only. Alex / Алекс is Strategy / Product Advisor. On 2026-09-24 he posted a signed strategy proposal to team-chat through the repository; access to that file does not make his proposed product changes accepted or give him a developer role. Denis / Денис and Igor / Игорь are Developers; Boris / Борис is QA Lead / Acceptance QA and does not fix application code. On 2026-09-25 Aleksey accepted [D013](decisions/D013-qa-acceptance-regression-split.md): a new Regression QA Engineer role handles routine smoke, UI/localization and bugfix retests with evidence while Boris owns critical acceptance. Aleksey has introduced the new agent as Nikita and reported a site login; Nikita has not yet documented two concurrent sessions or A↔B QA. [PM-036](tasks/PM-036-regression-qa-independent-two-accounts.md) corrects the earlier no-code discovery: the available Chrome profile is shared by agents, and the two current origin cookie jars belong to Boris. Two additional Workers have been deployed for Nikita's independent A/B pair; both now pass anonymous QA API checks after their Cloudflare `QA_WORKER_SECRET` bindings were configured. Backend QA access remains limited to verified immutable account IDs, with no broad QA privilege. The owner-approved `qa2`/`qa3` IDs are now provisioned in the existing allowlist; both verified accounts signed in on their dedicated origins and survived alternating reloads. The tester guide now names the working pair; message A↔B, logout/relogin and independent QA remain open. The existing qa/lelik pair belongs to Boris's unfinished acceptance and is not shared by default. Igor confirmed his name in team-chat on 2026-09-24; his earlier handoff remains under the identity he used at that time. Naming a participant does not claim a task or confirm that paused work has resumed. Model recommendations and the executor's pre-work check follow [ai-team.md](ai-team.md).

The owner accepted [D012](decisions/D012-documentation-current-history.md) on 2026-09-25: the task index, team-chat entrypoint and task cards expose current work, while the dated chat/task logs and changelog retain history. The pre-migration [task README snapshot](tasks/README-history-2026-09-25.md) and original chat at its linked Git revision remain available. Full archive reading is conditional on a specific history question; product-context and accepted decisions remain authoritative for product choices.
...
## QA drain and next product stage, 2026-09-25

[PM-034](tasks/PM-034-backend-frontend-refactoring-audit.md) is complete as a P2 read-only audit: Igor and Denis delivered separate backend/frontend reports, and Mark reconciled them without starting a rewrite. The backend currently has no upper bound for public search date ranges (no measured production load). [D014](decisions/D014-public-search-date-range.md) accepts **1–366 nights per public search request**, with 367+ rejected before SQL and no silent shortening of dates. [PM-037](tasks/PM-037-search-date-range-bound.md) is P2/done: Mark accepted the isolated small-pilot performance gate, Igor released backend and frontend with green CI, and Boris passed independent live QA. Raw production HTTP 400 was not independently read because Cloud Browser blocked direct API navigation. The 366-night boundary remains a search-request limit and any later change requires an explicit PM decision. This limits a PARROT search request, not rental duration. The frontend can replace unsaved host form drafts on language change or dashboard refresh (code-path finding, no independent reproduction yet), so [PM-038](tasks/PM-038-host-unsaved-drafts-rerender.md) is P2/NOW for focused QA followed by a narrow fix if confirmed. Search enrichment has 1+3N DB calls from code inspection, but no benchmark; transient dashboard errors may display an empty property list and need controlled QA. P1 search-to-contact acceptance and PM-029/036 remain first; no developer claim follows from these audit findings. [PM-030](tasks/PM-030-qa-drain-next-stage.md) reconciled 20 tasks previously in review; PM-029 has since joined the review queue (21 total) with partial PASS evidence and explicit remaining checks. [PM-029](tasks/PM-029-two-origin-qa-sessions.md) now has a working QA Worker/backend path returning 401 without a session; Boris independently completed the two-origin qa/lelik guest → host → guest smoke. During a later pair-block test, the main origin lost its visible `lelik` session; [PM-016](tasks/PM-016-qa-second-session-access.md) returned to in review and [PM-003](tasks/PM-003-contact-acceptance.md) is blocked until the test pair and host login are safely restored. [BUG-021](tasks/BUG-021-blocking-signs-out-host.md) records one unexplained observation; PM-029 still needs negative security and reverse logout checks. PM-017 (mailbox), PM-014 (native Safari), PM-015 (mobile session) and PM-018 (controlled calendar) require separate external resources. Do not infer acceptance from merges, owner-assisted checks or this plan.

[D015](decisions/D015-qa-environment-blocked-status.md) clarifies QA states: [PM-015](tasks/PM-015-qa-mobile-device-environment.md), [PM-007](tasks/PM-007-mobile-acceptance.md), [PM-025](tasks/PM-025-search-contact-button-layout.md) and [BUG-018](tasks/BUG-018-calendar-end-date-mobile-safari.md) are blocked on an independently controllable real iPhone Safari; [PM-014](tasks/PM-014-qa-native-autofill-environment.md) and [PM-004](tasks/PM-004-street-autofill.md) are blocked on a native saved-address Safari profile. Earlier desktop and owner-assisted PASS remain recorded. Mixed tasks with available desktop work stay in review with mobile criteria unverified. Mark has claimed [PM-039](tasks/PM-039-browser-test-environments.md), P1/NOW, to compare free automated browser emulation, Mac iOS Simulator and pay-per-minute remote real devices before recommending any expense; no service has been purchased or independently validated for Boris.

[D009](decisions/D009-host-cabinet-ia.md), [D010](decisions/D010-external-link-independent-verification.md) and [D011](decisions/D011-contact-always-available.md) remain the accepted contracts. PM-020/021 and related contact UI are implemented but still in review. For [PM-022](tasks/PM-022-public-host-profile.md), a minimal public host view is **proposed**, with field-level privacy questions for owner approval; the existing public JSON response does not authorize a new page or wider disclosure. [PM-031](tasks/PM-031-trust-terminology.md) prepares verifiable terminology; [PM-032](tasks/PM-032-how-it-works.md) and [PM-033](tasks/PM-033-faq.md) are later editorial tasks. Homepage restructuring and legal/privacy follow accepted public contracts and first QA evidence. Preserve developer capacity for a narrow confirmed bugfix until the first QA wave; PM-022 is only a conditional next dev candidate.
...
## PM-029: isolated QA origin (in review; security regression pending)

The QA alias is the existing production Worker URL `https://parrot669.cheltsov112.workers.dev`, alongside `https://parrot669.com` on the **same Worker**, backend and production database. Cloudflare's Worker Domains panel confirms both URLs already exist; adding `qa.parrot669.com` as a new custom domain failed in the dashboard, so it is only a future optional alias. Browser cookies have no `Domain` attribute, so each hostname maintains its own session. The Worker recognizes only these exact QA hostnames and sends a server-only attestation on proxied API requests; the backend rejects invalid attestations, requires an authenticated account for every QA API except login, and checks the account's immutable ID against the QA allowlist seeded from already verified `qa`/`lelik` accounts. QA login applies that ID check after password validation and before session issuance. Registration, email verification and password recovery API requests through the QA alias are closed. The QA Worker runs first for `/recover` and `/recover.html` (including Cloudflare's static asset canonical path), then redirects both to `https://parrot669.com/recover.html` with the language query so account owners can safely request recovery on the ordinary origin. Main-origin authorization remains unchanged. Backend PR #28 merged as `3e35c46`; both PR/main CI passed and Railway deployment `f9ebe8cf` is SUCCESS with Flyway V26. Valid Worker attestation with no/expired session now yields 401; invalid attestation and non-allowlisted accounts yield 403. On September 25 Aleksey restored the active production `QA_WORKER_SECRET` binding. Live anonymous `/api/auth/me` and `/api/locations/countries` through the QA Worker now reach Railway and return 401, proving the Worker and backend secrets match; QA Host/Messages show their login forms. Main `parrot669.com` still shows the separate `@lelik` session and loads locations. Boris independently confirmed two active qa/lelik sessions, A→B→A, refresh and logout `qa`. A later block test left `lelik` inaccessible in the QA browser and the pair blocked; PM-016 returned to in review, PM-003 is blocked and BUG-021 awaits safe reproduction. Third-account/forged-request checks and reverse logout remain in PM-029 review. See [PM-029](tasks/PM-029-two-origin-qa-sessions.md).
