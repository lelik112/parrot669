...
Mark / Марк is PM and edits product documentation only. Alex / Алекс is Strategy / Product Advisor. On 2026-09-24 he posted a signed strategy proposal to team-chat through the repository; access to that file does not make his proposed product changes accepted or give him a developer role. Denis / Денис and Igor / Игорь are Developers; Boris / Борис is QA Lead / Acceptance QA and does not fix application code. On 2026-09-25 Aleksey accepted [D013](decisions/D013-qa-acceptance-regression-split.md): a new Regression QA Engineer role handles routine smoke, UI/localization and bugfix retests with evidence while Boris owns critical acceptance. Aleksey has introduced the new agent as Nikita and reported a site login; Nikita has not yet documented two concurrent sessions or A↔B QA. Igor claimed PM-036 and showed the existing backend/Worker guard needs no code: with two browser contexts no allowlist change is required, while one context may require exact-ID entries in the existing allowlist after approval. QA tasks have not moved, and external environment blockers remain. [PM-036](tasks/PM-036-regression-qa-independent-two-accounts.md) plans a distinct two-account regression environment: check whether the agent controls two browser contexts, then provision a separate test pair; only if a single context forces the PM-029 QA origin should backend allowlist expansion be reviewed by exact IDs with security negative checks. The existing qa/lelik pair belongs to Boris's unfinished acceptance and is not shared by default. Igor confirmed his name in team-chat on 2026-09-24; his earlier handoff remains under the identity he used at that time. Naming a participant does not claim a task or confirm that paused work has resumed. Model recommendations and the executor's pre-work check follow [ai-team.md](ai-team.md).
