...
- Remaining backend extraction is explicitly LATER: auth routes, external calendar lifecycle, then legacy challenges in [PM-010](tasks/PM-010-auth-routes-extraction.md), [PM-011](tasks/PM-011-calendar-sync-extraction.md) and [PM-012](tasks/PM-012-legacy-challenges-extraction.md). Sequence and dependencies are in [D006](decisions/D006-backend-refactor-backlog.md); keep behavior fixes separate from mechanical moves.
