...
## BUG-022 — stable Host scroll return (production released; QA pending)
...
The narrow frontend fix retries the saved position during a bounded three-second layout-settling window, always clamps to the currently reachable maximum and stops as soon as the target is reached. Wheel, touch, pointer or keyboard input cancels restoration immediately, so it cannot fight a user who starts navigating. Ordinary reload, profile/property deep links, auth, drafts and API data are unchanged. A controller regression simulates the observed 3614 px late document growth and reaches the saved 2678 px. [PR #66](https://github.com/lelik112/parrot669/pull/66) merged as `844bb63`; PR CI [36229648159](https://github.com/lelik112/parrot669/actions/runs/36229648159) and main CI [36229732675](https://github.com/lelik112/parrot669/actions/runs/36229732675) passed, including 137 unit checks and 18 browser scenarios. Both `parrot669.com` and the Workers origin serve the new helper. Boris's independent two-origin production retest remains required before acceptance.
