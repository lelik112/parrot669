...
| --- | --- | --- | --- |
| [PM-001 — Проверка сохранённых дат](tasks/PM-001-calendar-control-dates.md) | P1 | Проверяется конкретное задание пользователя | Денис назначен; handoff Игоря снял пересечение с рефакторингом. Claim и старт Дениса ожидаются |
...
- Оставшиеся backend extraction этапы — [auth routes (PM-010)](tasks/PM-010-auth-routes-extraction.md), [calendar sync (PM-011)](tasks/PM-011-calendar-sync-extraction.md), [legacy challenges (PM-012)](tasks/PM-012-legacy-challenges-extraction.md). LATER: отдельными задачами после PM-001, без изменения поведения; исполнители не назначены. Уже выпущенные этапы не планировать повторно. Основание и порядок — [D006](decisions/D006-backend-refactor-backlog.md).
