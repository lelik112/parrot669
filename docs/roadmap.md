...
[PM-042](tasks/PM-042-agent-check-in-process-audit.md) закрыта как PM-аудит предложения Алекса после исправления коллизии ID с двухаккаунтной PM-036. [PM-043](tasks/PM-043-agent-wake-up-feasibility.md) — done: одноразовый wake-up и CAS доказаны; пятиминутный интервал и Markdown trigger недоступны в проверенной поверхности, массовый scheduler не включён. Игорь возвращается к PM-037 (Sol / High). Следующий шаг идеи автоматического запуска хранится в [PM-045](tasks/PM-045-five-minute-agent-wake-up-pilot.md), P2 / NEXT: Марк готовит пилот одного существующего чата в авторизованном UI; пяти минутная рекурсия, стоимость и rollout пока не подтверждены. Здесь есть встроенный планировщик, но его расписание не чаще часа, а GitHub-триггер не реагирует на изменение карточки. Желание проверять назначения каждые пять минут требует другой подтверждённой среды; приоритет P2, не отвлекать команду от QA основного пути.
...
- [PM-034](tasks/PM-034-backend-frontend-refactoring-audit.md) P2 — done: Игорь/Денис завершили read-only аудиты, Марк свёл их. [PM-037](tasks/PM-037-search-date-range-bound.md) поставлена на паузу до PM-043: замер малого пилота принят, release и QA ожидаются. PM-038 P2/NOW — отдельный QA черновиков и узкое исправление только после подтверждения. Скорость поискового enrichment и FE-3 5xx empty state остаются гипотезами для проверки.
