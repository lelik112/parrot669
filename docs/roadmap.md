...
[PM-042](tasks/PM-042-agent-check-in-process-audit.md) закрыта как PM-аудит предложения Алекса после исправления коллизии ID с двухаккаунтной PM-036. [PM-043](tasks/PM-043-agent-wake-up-feasibility.md) — done: одноразовый wake-up и CAS доказаны; пятиминутный интервал и Markdown trigger недоступны в проверенной поверхности, массовый scheduler не включён. Игорь возвращается к PM-037 (Sol / High). [PM-045](tasks/PM-045-five-minute-agent-wake-up-pilot.md) закрыта и отложена: по сообщению Алексея Игорь не смог настроить повтор каждые пять минут. Пилот и rollout не запускались. Открытая цель существующих чатов — [PM-046](tasks/PM-046-github-event-existing-chat-wakeup.md) P2 / NEXT: узко проверить обычный push и поддержанное событие PR; сквозного GitHub-пилота пока не было. Игорь предложен (Sol / Medium), claim не подтверждён; новый агент не входит в scope. Здесь есть встроенный планировщик, но его расписание не чаще часа, а GitHub-триггер не реагирует на изменение карточки. Желание проверять назначения каждые пять минут требует другой подтверждённой среды; приоритет P2, не отвлекать команду от QA основного пути.
