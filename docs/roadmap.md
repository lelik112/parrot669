...
| P2 | [PM-037](tasks/PM-037-search-date-range-bound.md) | Марк принял замер для малого пилота D014; Игорь (Sol / High) проверяет frontend PR #52 `mergeable=false`, актуальный CI, затем backend → frontend deploy и QA Бориса. Больший масштаб отдельно |
...
| P1 | [PM-040](tasks/PM-040-playwright-web-e2e.md) | Playwright выбран D016, planned: Денис — кандидат после BUG-020, dev claim нет, Sol / Medium; web smoke → QA Никиты/Бориса по ролям. [PM-039](tasks/PM-039-browser-test-environments.md) завершена как исследование |
...
Денис сейчас исправляет BUG-020; по прямому решению Алексея следующий P1 кандидат после handoff — узкое внедрение PM-040 с отдельным claim. PM-017 (почта) и PM-018 (контролируемый Airbnb) — отдельные ресурсы QA; [D015](decisions/D015-qa-environment-blocked-status.md) разделяет blocked и in review, [D016](decisions/D016-playwright-first-real-device-queue.md) задаёт Playwright-first и отдельную очередь настоящего телефона.
...
- [PM-034](tasks/PM-034-backend-frontend-refactoring-audit.md) P2 — done: Игорь/Денис завершили read-only аудиты, Марк свёл их. [PM-037](tasks/PM-037-search-date-range-bound.md) перешла в NOW: Игорь claim-ил реализацию, замер для малого пилота принят, ожидаются release и QA. PM-038 P2/NOW — отдельный QA черновиков и узкое исправление только после подтверждения. Скорость поискового enrichment и FE-3 5xx empty state остаются гипотезами для проверки.
